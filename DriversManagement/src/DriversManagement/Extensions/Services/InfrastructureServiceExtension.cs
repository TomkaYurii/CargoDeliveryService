...
public static class ServiceRegistration
{
    public static void AddInfrastructure(this IServiceCollection services, IWebHostEnvironment env, IConfiguration configuration)
    {
        ...
        var connectionString = configuration.GetConnectionStringOptions().DriversManagement;
        ...
        services.AddDbContext<DriversManagementContext>(options =>
            options.UseSqlServer(connectionString,
                builder => builder.MigrationsAssembly(typeof(DriversManagementContext).Assembly.FullName))
                            .UseSnakeCaseNamingConvention());

        services.AddHostedService<MigrationHostedService<DriversManagementContext>>();
