...
//}
    ...
    public async Task UpdateLast(ForumTopic topic)
    {
        var post = await _unitOfWork._postRepository.GetLastInTopic(topic.TopicID);
        await _unitOfWork._topicRepository.UpdateLastTimeAndUser(topic.TopicID, post.UserID, post.Name, post.PostTime);
        _unitOfWork.Commit();
    }
