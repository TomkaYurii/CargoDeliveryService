...
public class TopicRepository : GenericRepository<ForumPost>, ITopicRepository
{
    public TopicRepository(SqlConnection sqlConnection, IDbTransaction dbtransaction) : base(sqlConnection, dbtransaction, "pf_Topic")
    {
    ...
    public async Task UpdateLastTimeAndUser(int topicID, int userID, string name, DateTime postTime)
    {
        ...
        var updateQuery = "UPDATE pf_Topic SET LastPostUserID = @LastPostUserID, LastPostName = @LastPostName, LastPostTime = @LastPostTime WHERE TopicID = @TopicID";
        await _sqlConnection.ExecuteAsync(updateQuery,
            param: new { LastPostUserID = userID, LastPostName = name, LastPostTime = postTime, TopicID = topicID },
